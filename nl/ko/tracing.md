...
다음 튜토리얼에서는 Go 애플리케이션 추적을 위한 Opentracing 및 Jaeger 패키지에 초점을 맞춥니다. Jaeger 사용에 대한 자세한 정보는 [Jaeger 문서 포털](https://www.jaegertracing.io/docs/)을 참조하십시오.
...
Jaeger는 로컬 개발의 경우와 같이 Kubernetes 개발을 위해 올인원 서비스를 제공합니다. 프로덕션 코드가 아닌 개발을 위해서만 올인원 서비스를 사용하십시오. 프로덕션을 위해 Kubernetes에 배치하는 데 대해 자세히 보려면 [Jaeger Kubernetes 템플리트 안내서](https://github.com/jaegertracing/jaeger-kubernetes#production-setup)를 참조하십시오.
